...
module fftw3_mpi
    ...
    interface
        ! FFTW3 MPI functions
        function fftw_mpi_local_size_2d_transposed(n0, n1, comm, lH, lh0, lW, lw0) bind(C, name="fftw_mpi_local_size_2d_transposed")
            use, intrinsic :: iso_c_binding
            implicit none
        ...
        function fftw_mpi_plan_dft_r2c_2d(n0, n1, in, out, comm, flags) bind(C, name="fftw_mpi_plan_dft_r2c_2d")
            use, intrinsic :: iso_c_binding
            implicit none
        ...
        function fftw_mpi_plan_dft_c2r_2d(n0, n1, in, out, comm, flags) bind(C, name="fftw_mpi_plan_dft_c2r_2d")
            use, intrinsic :: iso_c_binding
            implicit none
...
module data_types
    ...
    type :: Arrays
        ...
        real(8) :: u_C_min               ! minimum of smoothed density (graphene)
        real(8) :: u_C_max               ! maximum
        real(8) :: u_BN_min              ! minimum of smoothed density (HBN)
...
! operator for linear part, e^{-k^2 \hat{\mathcal{L}} \Delta t} (graphene)
    ...
    type :: Model
        real(8) :: alpha_C               ! see documentation and/or input file
        real(8) :: alpha_BN
        real(8) :: alpha_C_B
        real(8) :: alpha_B_C
        real(8) :: alpha_C_N
    ...
    type :: Relaxation
        ...
        real(8) :: p_rex_BN
        real(8) :: d                     ! sampling step size for calculation box optimization

        integer :: T                     ! total number of iterations
        real(8) :: dx                    ! x-discretization
        real(8) :: dy                    ! y-discretization
        real(8) :: dt                    ! time step
        integer :: T_optimize            ! interval for calculation box optimization
    end type Relaxation
...
!        real(8) :: seed
        ...
        if (relax%iid == 0) then
            ...
            call MPI_Recv(seed, 1, MPI_INTEGER, relax%iid-1, 0, MPI_COMM_WORLD, status, ierr)
            call random_seed(put=seed)
            call random_number(seed)
            seed = int(seed * huge(seed))
        end if
        ...
        if (relax%iid /= relax%ID-1) then
            call MPI_Send(seed, 1, MPI_INTEGER, relax%iid+1, 0, MPI_COMM_WORLD, ierr)
        end if
    ...
    subroutine configure_arrays(arra, input)
        ...
        lWHh = fftw_mpi_local_size_2d_transposed(arra%H, arra%W/2+1, MPI_COMM_WORLD, arra%lH, arra%lh0, arra%lW, arra%lw0)
        lWHp = 2 * lWHh

        ! Allocate arrays
        arra%A_C = fftw_malloc(lWHh * sizeof(real(8)))
        arra%B_C = fftw_malloc(lWHh * sizeof(real(8)))
        arra%A_BN = fftw_malloc(lWHh * sizeof(real(8)))
        arra%B_BN = fftw_malloc(lWHh * sizeof(real(8)))

        arra%p_arr_C = fftw_malloc(lWHp * sizeof(real(8)))
        arra%q_arr_C = fftw_malloc(lWHp * sizeof(real(8)))
        arra%p_arr_B = fftw_malloc(lWHp * sizeof(real(8)))
        arra%q_arr_B = fftw_malloc(lWHp * sizeof(real(8)))
        arra%p_arr_N = fftw_malloc(lWHp * sizeof(real(8)))
        arra%q_arr_N = fftw_malloc(lWHp * sizeof(real(8)))

        arra%u_arr_C = fftw_malloc(lWHp * sizeof(real(8)))
        arra%u_arr_BN = fftw_malloc(lWHp * sizeof(real(8)))

        ! Set up FFTW plans
        arra%p_P_C = fftw_mpi_plan_dft_r2c_2d(arra%H, arra%W, arra%p_arr_C, arra%p_arr_C, &
                      MPI_COMM_WORLD, IOR(FFTW_MEASURE,FFTW_MPI_TRANSPOSED_OUT))
        arra%qa_Q_C = fftw_mpi_plan_dft_r2c_2d(arra%H, arra%W, arra%q_arr_C, arra%q_arr_C, &
 MPI_COMM_WORLD, FFTW_MEASURE.or.FFTW_MPI_TRANSPOSED_OUT)
        arra%Q_q_C = fftw_mpi_plan_dft_c2r_2d(arra%H, arra%W, arra%q_arr_C, arra%q_arr_C, &
MPI_COMM_WORLD, FFTW_MEASURE.or.FFTW_MPI_TRANSPOSED_IN)
        arra%p_P_B = fftw_mpi_plan_dft_r2c_2d(arra%H, arra%W, arra%p_arr_B, arra%p_arr_B, & 
MPI_COMM_WORLD, FFTW_MEASURE.or.FFTW_MPI_TRANSPOSED_OUT)
        arra%qa_Q_B = fftw_mpi_plan_dft_r2c_2d(arra%H, arra%W, arra%q_arr_B, arra%q_arr_B, & 
MPI_COMM_WORLD, FFTW_MEASURE.or.FFTW_MPI_TRANSPOSED_OUT)
        arra%Q_q_B = fftw_mpi_plan_dft_c2r_2d(arra%H, arra%W, arra%q_arr_B, arra%q_arr_B, &
 MPI_COMM_WORLD, FFTW_MEASURE.or.FFTW_MPI_TRANSPOSED_IN)
        arra%p_P_N = fftw_mpi_plan_dft_r2c_2d(arra%H, arra%W, arra%p_arr_N, arra%p_arr_N, & 
MPI_COMM_WORLD, FFTW_MEASURE.or.FFTW_MPI_TRANSPOSED_OUT)
        arra%q_Q_N = fftw_mpi_plan_dft_r2c_2d(arra%H, arra%W, arra%q_arr_N, arra%q_arr_N, & 
MPI_COMM_WORLD, FFTW_MEASURE.or.FFTW_MPI_TRANSPOSED_OUT)
        arra%Q_q_N = fftw_mpi_plan_dft_c2r_2d(arra%H, arra%W, arra%q_arr_N, arra%q_arr_N, & 
MPI_COMM_WORLD, FFTW_MEASURE.or.FFTW_MPI_TRANSPOSED_IN)

        arra%ua_U_C = fftw_mpi_plan_dft_r2c_2d(arra%H, arra%W, arra%u_arr_C, arra%u_arr_C, & 
MPI_COMM_WORLD, FFTW_MEASURE.or.FFTW_MPI_TRANSPOSED_OUT)
        arra%U_u_C = fftw_mpi_plan_dft_c2r_2d(arra%H, arra%W, arra%u_arr_C, arra%u_arr_C, & 
MPI_COMM_WORLD, FFTW_MEASURE.or.FFTW_MPI_TRANSPOSED_IN)
        arra%ua_U_BN = fftw_mpi_plan_dft_r2c_2d(arra%H, arra%W, arra%u_arr_BN, arra%u_arr_BN, & 
MPI_COMM_WORLD, FFTW_MEASURE.or.FFTW_MPI_TRANSPOSED_OUT)
        arra%U_u_BN = fftw_mpi_plan_dft_c2r_2d(arra%H, arra%W, arra%u_arr_BN, arra%u_arr_BN, & 
MPI_COMM_WORLD, FFTW_MEASURE.or.FFTW_MPI_TRANSPOSED_IN)
    ...
    subroutine configure_model(mmode, input)
        ...
        read(input, *) mmode%alpha_C, mmode%beta_C, mmode%gamma_C_l, mmode%gamma_C_s, mmode%delta_C
        read(input, *) mmode%alpha_BN, mmode%beta_BN, mmode%gamma_BN_l, mmode%gamma_BN_s, mmode%delta_BN
    ...
    subroutine configure_relaxation(relax, input)
        ...
        read(input, *) relax%T, relax%dx, relax%dy, relax%dt, relax%T_optimize
...
(gh - 0.5d0 * arra%H) * dy, 0.5d0 * l_BN * l0 + u_arr_BN, -0.5d0 * l_BN * l0 / sqrt(3.0d0) + v_BN, l_BN * l0, theta_BN)
...
(gh - 0.5d0 * arra%H) * dy, u_arr_C, v_C, l0, theta_C)
    ...
    subroutine write_state(arra, relax, outp)
        ...
        Wp = 2 * (arra%W / 2 + 1)
        write(filename, '(A, "_t:", I0, ".dat")') trim(outp%name), relax%tt

        do i = 0, relax%ID - 1
            call MPI_Barrier(MPI_COMM_WORLD, ierr)    ! synchronize processes
            if (relax%iid == i) then
                if (relax%iid == 0) then
                    open(file, file=filename, status='unknown', action='write', iostat=ierr)
                else
                    open(file, file=filename, status='unknown', position='append', iostat=ierr)
                end if
                do h = 1, arra%lH
                    k = Wp * (h - 1)
                    do w = 1, arra%W
                        v_C = map(arra%u_arr_BN(k + w), arra%u_BN_max, arra%u_BN_min)
                        v_BN = map(arra%u_arr_C(k + w), arra%u_C_min, arra%u_C_max)
                        dv = (v_C - v_BN) / (v_C + v_BN)
                        write(file, '(6E20.12)') arra%q_arr_C(k + w), arra%q_arr_B(k + w), &
    ...
    subroutine print_output(relax, outp)
        ...
        if (relax%iid == 0) then
            print *, relax%tt, int(time() - relax%t0), relax%dx, relax%dy, &
                relax%f_C + relax%f_BN, relax%f_C, relax%f_BN, relax%p_rex_C, relax%p_rex_BN
            write(filename, '(A, ".out")') trim(outp%name)
            open(file, file=filename, status='unknown', position='append', iostat=ierr)
            write(file, '(I0, I0, 6E20.12)') relax%tt, int(time() - relax%t0), relax%dx, relax%dy, &
                relax%f_C + relax%f_BN, relax%f_C, relax%f_BN, relax%p_rex_C, relax%p_rex_BN
            close(file)
        end if
...
q_fp_C, q_fp_B, q_fp_N
        real(8) :: p_C2, p_B2, p_N2, v_C, v_BN, gamma_C, gamma_BN, dv, mask
        ...
        do h = 1, arra%lH
            ...
            do w = 1, arra%W
                ...
                v_C = map(arra%u_arr_BN(k + w), arra%u_BN_max, arra%u_BN_min)
                v_BN = map(arra%u_arr_C(k + w), arra%u_C_min, arra%u_C_max)
                gamma_C = v_C * (mmode%gamma_C_s - mmode%gamma_C_l) + mmode%gamma_C_l
                gamma_BN = v_BN * (mmode%gamma_BN_s - mmode%gamma_BN_l) + mmode%gamma_BN_l
                dv = (v_C - v_BN) / (v_C + v_BN)
                mask = exp(a * dv * dv)
 ...
 div3 * gamma_C * p_fp_C * p_C2 + 0.25d0 * delta_C * p_C2 * p_C2 + &
                    mask * (alpha_C_B * p_fp_C * p_fp_B + 0.5d0 * gamma_C_B * (p_C2 * p_fp_B + p_fp_C * p_B2) & 
+ alpha_C_N * p_fp_C * p_fp_N + 0.5d0 * gamma_C_N * (p_C2 * p_fp_N + p_fp_C * p_N2))
...
+ div3 * gamma_BN * p_fp_B * p_B2 + 0.25d0 * delta_BN * p_B2 * p_B2 + &
                    0.5d0 * alpha_BN * p_N2 + 0.5d0 * beta_BN * p_fp_N * q_fp_N & 
+ div3 * gamma_BN * p_fp_N * p_N2 + 0.25d0 * delta_BN * p_N2 * p_N2 + &
                    alpha_B_N * p_fp_B * p_fp_N + beta_B_N * p_fp_B * q_fp_N & 
+ 0.5d0 * gamma_B_N * (p_B2 * p_fp_N + p_fp_B * p_N2) + &
                    mask * (alpha_B_C * p_fp_B * p_fp_C + 0.5d0 * gamma_B_C * (p_B2 * p_fp_C + p_fp_B * p_C2) & 
+ alpha_N_C * p_fp_N * p_fp_C + 0.5d0 * gamma_N_C * (p_N2 * p_fp_C + p_fp_N * p_C2))
        ...
        call MPI_Allreduce(MPI_IN_PLACE, relax%f_C, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD)
        call MPI_Allreduce(MPI_IN_PLACE, relax%f_BN, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD)
        call MPI_Allreduce(MPI_IN_PLACE, relax%p_rex_C, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD)
        call MPI_Allreduce(MPI_IN_PLACE, relax%p_rex_BN, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD)
    ...
    subroutine update_us(arra, mmode, relax)
        ...
        do h = 1, arra%lH
            ...
            do w = 1, arra%W
                ...
                arra%u_arr_BN(k + w) = 0.5d0 * (arra%q_arr_B(k + w) + arra%q_arr_N(k + w))
            end do
        end do
        ...
        do h = 1, arra%lH
            k = Wp * (h - 1)
            do w = 1, arra%W
                if (arra%u_arr_C(k + w) < arra%u_C_min) arra%u_C_min = arra%u_arr_C(k + w)
                if (arra%u_arr_C(k + w) > arra%u_C_max) arra%u_C_max = arra%u_arr_C(k + w)
                if (arra%u_arr_BN(k + w) < arra%u_BN_min) arra%u_BN_min = arra%u_arr_BN(k + w)
                if (arra%u_arr_BN(k + w) > arra%u_BN_max) arra%u_BN_max = arra%u_arr_BN(k + w)
            end do
        end do

        call MPI_Allreduce(MPI_IN_PLACE, arra%u_C_min, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD)
        call MPI_Allreduce(MPI_IN_PLACE, arra%u_C_max, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD)
        call MPI_Allreduce(MPI_IN_PLACE, arra%u_BN_min, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD)
        call MPI_Allreduce(MPI_IN_PLACE, arra%u_BN_max, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD)
    ...
    subroutine step(arra, mmode, relax)
        ...
        real(8) :: delta_BN, alpha_B_N, beta_B_N, gamma_B_N, alpha_B_C, gamma_B_C
        real(8) :: alpha_N_C, gamma_N_C
        ...
        do h = 1, arra%lH
            ...
            do w = 1, arra%W
                ...
                arra%q_arr_C(k + w) = gamma_C * q_C2 + delta_C * q_step_C * q_C2 + &
                    mask * (alpha_C_B * q_step_B + gamma_C_B * (q_step_C * q_step_B + 0.5d0 * q_B2) & 
+ alpha_C_N * q_step_N + gamma_C_N * (q_step_C * q_step_N + 0.5d0 * q_N2))
                ! (B)
                arra%q_arr_B(k + w) = gamma_BN * q_B2 + delta_BN * q_step_B * q_B2 & 
+ alpha_B_N * q_step_N + gamma_B_N * (q_step_B * q_step_N + 0.5d0 * q_N2) + &
                    mask * (alpha_B_C * q_step_C + gamma_B_C * (q_step_B * q_step_C + 0.5d0 * q_C2))
                ! (N)
                arra%q_arr_N(k + w) = gamma_BN * q_N2 + delta_BN * q_step_N * q_N2 & 
+ alpha_B_N * q_step_B + gamma_B_N * (q_step_N * q_step_B + 0.5d0 * q_B2) + &
                    mask * (alpha_N_C * q_step_C + gamma_N_C * (q_step_N * q_step_C + 0.5d0 * q_C2))
            end do
        end do
        ...
        do w = 1, arra%lW
            ...
            k = arra%H * (w - 1)
            do h = 1, arra%H
                P_B_prev = P_B(k + h)    ! P_B(k + h) will be overwritten so value saved for computing P_N(k + h)
                if (h <= arra%H / 2) then
                    ky = (h - 1) * dky
                else
                    ky = (h - 1 - arra%H) * dky
                ...
                P_C(k + h) = arra%A_C(k + h) * P_C(k + h) + arra%B_C(k + h) * Q_C(k + h)
                ! \psi_B and \psi_N
                d2 = q2 - k2
                d2 = d2 * d2
...
+ arra%B_BN(k + h) * (Q_N(k + h) + beta_B_N * d2 * P_B_prev * WH)
    ...
    subroutine optimize(arra, mmode, relax)
        ...
        dxs = [relax%dx, relax%dx-relax%d, relax%dx+relax%d, relax%dx, relax%dx]
        dys = [relax%dy, relax%dy, relax%dy, relax%dy-relax%d, relax%dy+relax%d]
        ...
        relax%dx = (relax%d * (fs(2) - fs(3)) & 
+ 2.0d0 * dxs(1) * (-2.0d0 * fs(1) + fs(2) + fs(3))) / (2.0d0 * (fs(2) - 2.0d0 * fs(1) + fs(3)))
        relax%dy = (relax%d * (fs(4) - fs(5)) & 
+ 2.0d0 * dys(1) * (-2.0d0 * fs(1) + fs(4) + fs(5))) / (2.0d0 * (fs(4) - 2.0d0 * fs(1) + fs(5)))
    ...
    subroutine relaxx(arra, mmode, relax, outp)
        ...
        do relax%tt = 0, relax%T
            if (relax%T_optimize > 0 .and. relax%tt > 0 .and. mod(relax%tt, &
 relax%T_optimize) == 0) call optimize(arra, mmode, relax)
            if (mod(relax%tt, outp%T_print) == 0) then
                call fp(arra, mmode, relax)
                call print_output(relax, outp)
            end if
            if (mod(relax%tt, outp%T_write) == 0) call write_state(arra, relax, outp)
            if (relax%tt < relax%T) call step(arra, mmode, relax)
        end do
    ...
    subroutine clear_arrays(arra)
        ...
        call fftw_free(arra%p_arr_C)
        call fftw_free(arra%q_arr_C)
        call fftw_free(arra%p_arr_B)
        call fftw_free(arra%q_arr_B)
        call fftw_free(arra%p_arr_N)
        call fftw_free(arra%q_arr_N)
        call fftw_free(arra%u_arr_C)
        call fftw_free(arra%u_arr_BN)
        call fftw_free(arra%A_C)
        call fftw_free(arra%B_C)
        call fftw_free(arra%A_BN)
        call fftw_free(arra%B_BN)
...
!    use mpi_f08
 !   include 'fftw3.f03'
    ...
    open(unit=input, file=filename, status='old', action='read', iostat=ierr)
    if (ierr /= 0) then
